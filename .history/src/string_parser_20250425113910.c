    ...
    while (stack.top >= 0 && !error) {
        ...
        if (isTerminal(stackTop) && strcmp(stackTop, "$") != 0) {
            if (matchesTerminal(stackTop, tokens[inputIndex])) {
                ...
                printf("Line %d: Syntax Error: Unexpected token '%s'. Expected '%s'.\n", inputIndex + 1, tokens[inputIndex], stackTop);
                error = true;
                break;
            }
        }
        ...
        // Case 2: Stack top is a non-terminal
            ...
            if (found && !entry->isError) {
                ...
                printf("%-30s\n", "Error: No table entry for non-terminal");
                printf("Line %d: Syntax Error: No valid production for [%s, %s]\n", inputIndex + 1, stackTop, tokens[inputIndex]);
                error = true;
                break;
            }
        }
        // Case 3: Stack top is $
            ...
            printf("Line %d: Syntax Error: Invalid symbol in stack.\n", inputIndex + 1);
            error = true;
            break;
        }
    }
