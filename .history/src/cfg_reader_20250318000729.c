...
void simplifyCFG() {
    for (int i = 0; i < productionCount; i++) {
        ...
        char *rhsCopy = malloc(strlen(rhs));
        strcpy(rhsCopy, rhs);

        char *token = strtok(rhsCopy, "|");  // Split by '|'
        int first = 1;
        ...
        while (token) {
            ...
            if (first) {
                // Replace original rule's RHS with the first token
                strcpy(grammar[i].rhs, token);
                first = 0;
            } else {
                // Add a new production for the same LHS
                if (productionCount >= maxProductions) {
                    maxProductions *= 2;
                    grammar = (Production*)realloc(grammar, maxProductions * sizeof(Production));
                }

                strcpy(grammar[productionCount].lhs, lhs);
                grammar[productionCount].rhs = malloc(strlen(token) + 1);
                strcpy(grammar[productionCount].rhs, token);
                productionCount++;
            }
