...
void removeIndirectLeftRecursion() {
    for (int i = 0; i < productionCount; i++) {
        for (int j = 0; j < i; j++) {
            if (strcmp(grammar[i].lhs, grammar[j].lhs) == 0) continue;

            // Split grammar[i].rhs into tokens
            char *tokens[100];
            int tokenCount = 0;
...
char *token = strtok(rhsCopy, "|");
while (token) {
    char *rhsTrimmed = trim(token);
    if (strncmp(rhsTrimmed, grammar[j].lhs, strlen(grammar[j].lhs)) == 0 &&
        (rhsTrimmed[strlen(grammar[j].lhs)] == ' ' || rhsTrimmed[strlen(grammar[j].lhs)] == '\0')) {

        // Starts with Aj, so indirect left recursion present
        for (char *sub = strtok(grammar[j].rhs, "|"); sub != NULL; sub = strtok(NULL, "|")) {
            char replaced[1000] = "";
            snprintf(replaced, sizeof(replaced), "%s%s", trim(sub), rhsTrimmed + strlen(grammar[j].lhs));
            strcat(newRHS, replaced);
            strcat(newRHS, " | ");
        }
    } else {
        strcat(newRHS, rhsTrimmed);
        strcat(newRHS, " | ");
    }
    token = strtok(NULL, "|");
}
            ...
            // Start replacing indirect left recursion A -> Bγ where B is grammar[j].lhs
            ...
            for (int k = 0; k < tokenCount; k++) {
                if (strncmp(tokens[k], grammar[j].lhs, strlen(grammar[j].lhs)) == 0) {
                    ...
                    while (*suffix == ' ') suffix++;  // Trim leading spaces

                    // Replace B with all RHSs of B
                    char *rhs_j_copy = strdup(grammar[j].rhs);
                    char *subToken = strtok(rhs_j_copy, "|");
                    while (subToken) {
                        while (*subToken == ' ') subToken++; // Trim
        ...
        // After indirect left recursion removal, remove direct left recursion
