...
bool isTerminal(const char* symbol) {
    ...
    // Check if the symbol is an identifier (id)
    // We assume identifiers start with a letter and can contain letters/numbers
    if (isalpha(symbol[0])) {
        return true;  // It's a terminal because it's an identifier
    }
