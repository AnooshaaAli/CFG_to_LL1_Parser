...
void performLeftFactoring() {
    for (int i = 0; i < productionCount; i++) {
        ...
        for (int j = i + 1; j < productionCount; j++) {
            if (strcmp(grammar[j].lhs, lhs) == 0) {  // ✅ Correct
                ...
                if (k > 0) {
                    ...
                    snprintf(grammar[i].rhs, strlen(newRhs) + 2, "%s%c", newRhs, newNonTerminal);
                    grammar[j].lhs = newNonTerminal;
