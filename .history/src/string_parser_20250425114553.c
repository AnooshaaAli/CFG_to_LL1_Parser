...
// Error structure for syntax errors
typedef struct {
    int line_num;
    char error[100];
} SyntaxErrors;
...
void parseString(ParseTable* table, const char* input, const char* startSymbol) {
    ...
    // Syntax error storage
    SyntaxErrors syntaxErrors[MAX_SYNTAX_ERRORS];
    int errorCount = 0;
    ...
    while (stack.top >= 0 && !error) {
        ...
        if (isTerminal(stackTop) && strcmp(stackTop, "$") != 0) {
            if (matchesTerminal(stackTop, tokens[inputIndex])) {
                ...
                // Store error in syntaxErrors
                syntaxErrors[errorCount].line_num = inputIndex + 1;
                snprintf(syntaxErrors[errorCount].error, sizeof(syntaxErrors[errorCount].error), "Unexpected token '%s' after '%s'", tokens[inputIndex], stackTop);
                errorCount++;
                error = true;
                break;
            }
        ...
        // Case 2: Stack top is a non-terminal
            ...
            if (found && !entry->isError) {
                ...
                // Store error in syntaxErrors
                syntaxErrors[errorCount].line_num = inputIndex + 1;
                snprintf(syntaxErrors[errorCount].error, sizeof(syntaxErrors[errorCount].error), "No table entry for [%s, %s]", stackTop, tokens[inputIndex]);
                errorCount++;
                error = true;
                break;
            }
