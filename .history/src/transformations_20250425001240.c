...
void performLeftFactoring() {
    ...
    for (int i = 0; i < productionCount; i++) {
        ...
        while (token != NULL) {
            // Trim spaces and store each part of the rhs
            while (*token == ' ') token++;
            char *end = token + strlen(token) - 1;
            while (end > token && *end == ' ') {
                *end = '\0'; 
        ...
        // Handling left recursion: Split off direct recursion cases
        int prefixLength = 0;
        if (strncmp(tokens[0], "Expr", 4) == 0) {
            prefixLength = 4; // We know it's an Expr-related prefix (id = Expr | etc.)
        }

        if (prefixLength > 0 && tokenCount > 1) {
            // Create new production for the left-factored part
            char newLHS[100];
            ...
            // Add new production for the factored rules
            Production newProduction;
            strcpy(newProduction.lhs, newLHS);
            newProduction.rhs = malloc(strlen(rhs) + 1);
            newProduction.rhs[0] = '\0';

            for (int j = 1; j < tokenCount; j++) {
                strcat(newProduction.rhs, tokens[j]);
                if (j < tokenCount - 1) strcat(newProduction.rhs, " | ");
            }
