...
void performLeftFactoring() {
    ...
    for (int i = 0; i < productionCount; i++) {
        ...
        // Tokenize the right-hand side of the production using "|"
        char *tokens[100];
        int tokenCount = 0;
        ...
        // Find length of the longest common prefix between the alternatives
        int prefixLength = strlen(tokens[0]);
        for (int j = 1; j < tokenCount; j++) {
            int k = 0;
            while (tokens[j][k] && tokens[0][k] && tokens[j][k] == tokens[0][k]) {
            ...
            // Update prefix length to the shortest common prefix
            prefixLength = (k < prefixLength) ? k : prefixLength;
        }

        // If a common prefix exists, perform left factoring
        if (prefixLength > 0 && tokenCount > 1) {
            // Create a new non-terminal for the factored part
            char newLHS[100];
            snprintf(newLHS, sizeof(newLHS), "%s'", lhs);

            // Modify the original production to use the new non-terminal
            char *newRhs = malloc(prefixLength + strlen(newLHS) + 2);
            strncpy(newRhs, tokens[0], prefixLength);
            newRhs[prefixLength] = '\0';
            strcat(newRhs, newLHS);
            ...
            Production newProduction;
            strcpy(newProduction.lhs, newLHS);
            newProduction.rhs = malloc(strlen(rhs) - prefixLength + 2);
            newProduction.rhs[0] = '\0';

            // Add the remaining parts of the alternatives after the common prefix
            for (int j = 0; j < tokenCount; j++) {
                if (tokens[j][prefixLength] == '\0') {
                    strcat(newProduction.rhs, "ε");  // epsilon for empty alternative
                } else {
                    strcat(newProduction.rhs, tokens[j] + prefixLength);  // remaining part after common prefix
                }

                if (j < tokenCount - 1) strcat(newProduction.rhs, " | ");  // separate alternatives
            }

            // Add the new production rule to the grammar
            grammar[productionCount] = newProduction;
            productionCount++;
        }
    ...
    // Simplify the CFG again after left factoring
    simplifyCFG();
}
