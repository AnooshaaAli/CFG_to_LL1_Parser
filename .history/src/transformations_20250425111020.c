...
void performLeftFactoring() {
    ...
    for (int i = 0; i < productionCount; i++) {
        ...
        for (int j = 1; j < tokenCount; j++) {
            ...
            // Handles cases like (S -> id | id +=) for correct prefix length
        ...
        // If we have a common prefix, we need to perform left factoring
        if (prefixLength > 0 && tokenCount > 1) {
            ...
            // Modify the original production to have the common prefix and the new non-terminal
            char *newRhs = malloc(prefixLength + strlen(newLHS) + 2);
            strncpy(newRhs, tokens[0], prefixLength);
            newRhs[prefixLength] = '\0';
            strcat(newRhs, newLHS);
            ...
            for (int j = 0; j < tokenCount; j++) {
                // For each token, add the part after the common prefix
        ...
        // Free up the memory for the tokens
        for (int j = 0; j < tokenCount; j++) {
            free(tokens[j]);
        }
    }
